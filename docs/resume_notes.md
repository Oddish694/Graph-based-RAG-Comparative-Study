...
- 在 100 条 HotpotQA validation 样本上完成公平对比：Hybrid RAG 取得 Recall@5 0.790、Full Evidence Recall@5 0.590、NDCG@5 0.7582，优于 Vector RAG 和 BM25。进一步实现增强版 GraphRAG-style retriever，通过句子级带权实体图、实体频率加权和一跳距离衰减将 Recall@10 提升到 0.920、Full Evidence Recall@10 提升到 0.840；在此基础上实现 Improved LightRAG-style coverage reranking，将 Recall@5 提升到 0.825、Full Evidence Recall@5 提升到 0.660、NDCG@10 提升到 0.8209，并通过消融实验验证图扩展和 coverage reranking 的模块贡献。
...
- Evaluated on 100 HotpotQA validation samples. Hybrid RAG achieved Recall@5 of 0.790, Full Evidence Recall@5 of 0.590 and NDCG@5 of 0.7582. An enhanced GraphRAG-style retriever improved Recall@10 to 0.920 and Full Evidence Recall@10 to 0.840 through a sentence-level weighted entity graph, entity-frequency weighting and one-hop distance decay. Improved LightRAG-style coverage reranking improved Recall@5 to 0.825, Full Evidence Recall@5 to 0.660 and NDCG@10 to 0.8209, with ablation experiments validating graph expansion and coverage reranking contributions.
...
Phase 3 我实现并升级了一个轻量 GraphRAG-style retriever。它不是完整复现复杂的 GraphRAG 系统，而是验证图增强检索的核心思想：实体关系图能不能帮助多跳问答找回 Hybrid RAG 漏掉的 supporting evidence。

实现上，我先用增强规则抽取专有名词、缩写、标题短语和谨慎别名，例如 `Ada Lovelace`、`Analytical Engine`、`Charles Babbage`、`U.S.` 以及引号里的作品名。然后构建 `GraphIndex`，维护 entity-to-chunk、chunk-to-entity、句子级带权 entity edges 和 edge evidence。检索时先用 Hybrid RAG 找 seed chunks，再从 query entities 和高信息量 seed entities 出发做一跳图邻居扩展，最后融合 seed score 和 graph score。

评分上，我没有继续用简单实体重合比例，而是把图分数拆成 `query_entity_score`、`seed_entity_score` 和 `graph_proximity_score`。其中实体权重按出现频率反比计算，稀有实体权重更高；图邻居使用距离衰减，距离越远贡献越低；同时用 `max_seed_entities` 限制 seed 侧扩展范围，避免图扩展过宽。每个检索结果还会记录 `graph_paths`，包括路径实体、边权和边证据，方便后续 case study。

在 100 条 HotpotQA validation fair baseline 设置下，GraphRAG-style 的 Recall@5 达到 0.810，Full Evidence Recall@5 达到 0.630，NDCG@5 达到 0.7730；Top-10 上 Recall@10 达到 0.920，Full Evidence Recall@10 达到 0.840。相比 Hybrid RAG，图检索能找到更多完整证据链，但平均检索延迟也从 0.0213s 增加到 0.1355s。因此后续需要继续分析效果与成本权衡。
...
在 100 条 HotpotQA validation fair baseline 设置下，Improved LightRAG 的 Recall@5 达到 0.825，Full Evidence Recall@5 达到 0.660，NDCG@5 达到 0.7834；Top-10 上 Recall@10 达到 0.920，Full Evidence Recall@10 达到 0.840，NDCG@10 达到 0.8209。这说明 coverage reranking 在增强图候选池上不只是改善排序，也能把完整证据链推进 top-5。

消融实验也验证了模块贡献：关闭图扩展后结果基本回到 Hybrid RAG；关闭 coverage reranking 后结果回到 GraphRAG-style；关闭别名后图规模下降，但 top-5 完整证据召回也下降。因此当前 Phase 3 更准确的定位是“带有稀有实体加权、句子级带权边和一跳距离衰减的轻量图感知检索 baseline”。
