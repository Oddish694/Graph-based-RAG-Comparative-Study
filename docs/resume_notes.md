...
- 在 100 条 HotpotQA validation 样本上完成公平对比：Hybrid RAG 取得 Recall@5 0.790、Full Evidence Recall@5 0.590、NDCG@5 0.7582，优于 Vector RAG 和 BM25。进一步实现增强版 GraphRAG-style retriever，通过实体图邻居扩展、实体频率加权和图距离衰减将 Recall@10 提升到 0.905、Full Evidence Recall@10 提升到 0.810；在此基础上实现 Improved LightRAG-style coverage reranking，将 Recall@5 提升到 0.820、Full Evidence Recall@5 提升到 0.650、NDCG@10 提升到 0.8209，并补充 LightRAG controlled integration layer，为后续外部方法对照提供统一评估接口。
...
- Evaluated on 100 HotpotQA validation samples. Hybrid RAG achieved Recall@5 of 0.790, Full Evidence Recall@5 of 0.590 and NDCG@5 of 0.7582. An enhanced GraphRAG-style retriever improved Recall@10 to 0.905 and Full Evidence Recall@10 to 0.810 through entity-graph neighbor expansion, entity-frequency weighting and graph-distance decay. Improved LightRAG-style coverage reranking improved Recall@5 to 0.820, Full Evidence Recall@5 to 0.650 and NDCG@10 to 0.8209, with a LightRAG controlled integration layer added for future external method comparison.
...
实现上，我先用增强规则抽取专有名词、缩写、标题短语和谨慎别名，例如 `Ada Lovelace`、`Analytical Engine`、`Charles Babbage`、`U.S.` 以及引号里的作品名。然后构建 `GraphIndex`，维护 entity-to-chunk、chunk-to-entity 和 entity co-occurrence graph。检索时先用 Hybrid RAG 找 seed chunks，再从 query entities 和高信息量 seed entities 出发做图邻居扩展，最后融合 seed score 和 graph score。

评分上，我没有继续用简单实体重合比例，而是把图分数拆成 `query_entity_score`、`seed_entity_score` 和 `graph_proximity_score`。其中实体权重按出现频率反比计算，稀有实体权重更高；图邻居使用距离衰减，距离越远贡献越低；同时用 `max_seed_entities` 限制 seed 侧扩展范围，避免图扩展过宽。

在 100 条 HotpotQA validation fair baseline 设置下，GraphRAG-style 的 Recall@5 达到 0.805，Full Evidence Recall@5 达到 0.620，NDCG@5 达到 0.7727；Top-10 上 Recall@10 达到 0.905，Full Evidence Recall@10 达到 0.810。相比 Hybrid RAG，图检索能找到更多完整证据链，但平均检索延迟也从 0.0213s 增加到 0.0866s。因此下一阶段需要继续通过 coverage-aware reranking 和消融实验分析效果与成本权衡。
...
在 100 条 HotpotQA validation fair baseline 设置下，Improved LightRAG 的 Recall@5 达到 0.820，Full Evidence Recall@5 达到 0.650，NDCG@5 达到 0.7851；Top-10 上 Recall@10 达到 0.910，Full Evidence Recall@10 达到 0.820，NDCG@10 达到 0.8209。这说明 coverage reranking 在增强图候选池上不只是改善排序，也能把完整证据链推进 top-5。下一步需要通过消融实验验证 enhanced graph scoring、graph expansion 和 coverage reranking 各自的贡献。
