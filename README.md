...
- Phase 5: 100-sample ablation matrix for graph expansion, aliases and coverage reranking。
...
- 轻量 GraphRAG-style retriever：规则实体抽取、句子级带权 entity co-occurrence graph、稀有实体加权、图距离衰减、graph path tracing。
...
结论：Hybrid RAG 是强 baseline；句子级带权图让 GraphRAG-style 在 top-5 和 top-10 证据覆盖上超过 Hybrid。Improved LightRAG 通过 coverage-aware reranking 将 Recall@5 提升到 0.825、Full Evidence Recall@5 提升到 0.660、NDCG@10 提升到 0.8209。Phase 5 消融显示：关闭图扩展后结果回到 Hybrid 附近，关闭 coverage reranking 后结果回到 GraphRAG-style 附近。LightRAG controlled 当前使用 `local_compat` 后端，作用是验证外部 LightRAG 对照组的统一接入流程，不能当作官方 LightRAG 最终成绩。
...
configs/
  ...
  phase5_improved_lightrag_no_aliases.yaml
docs/
...
> 构建面向 HotpotQA 多跳问答的可复现 RAG 对比实验框架，统一数据标准化、文本切块、Vector RAG、BM25 关键词检索、Hybrid score fusion 和多维检索指标评估。在统一 chunking、top-k 和数据配置下完成 Vector / BM25 / Hybrid fair baseline 对比，并新增 Precision@k、NDCG@k、Full Evidence Recall@k 等多跳证据完整性指标。进一步实现增强版 GraphRAG-style retriever，通过规则实体抽取、句子级带权 entity graph、稀有实体加权和一跳距离衰减，将 Recall@10 提升到 0.920、Full Evidence Recall@10 提升到 0.840；实现 Improved LightRAG-style coverage reranking，将 Recall@5 提升到 0.825、Full Evidence Recall@5 提升到 0.660、NDCG@10 提升到 0.8209，并通过消融实验验证图扩展和 coverage reranking 的模块贡献。

English:

> Built a reproducible RAG comparison framework for HotpotQA multi-hop QA, including dataset normalization, chunking, Vector RAG, BM25 retrieval, hybrid score fusion and retrieval metrics. Established fair Vector / BM25 / Hybrid baselines under unified chunking, top-k and dataset settings, and added multi-hop evidence metrics such as Precision@k, NDCG@k and Full Evidence Recall@k. An enhanced GraphRAG-style retriever improved Recall@10 to 0.920 and Full Evidence Recall@10 to 0.840 through rule-based entity extraction, a sentence-level weighted entity graph, rare-entity weighting and one-hop distance decay. An Improved LightRAG-style coverage reranker improved Recall@5 to 0.825, Full Evidence Recall@5 to 0.660 and NDCG@10 to 0.8209, with ablation experiments validating graph expansion and coverage reranking contributions.
